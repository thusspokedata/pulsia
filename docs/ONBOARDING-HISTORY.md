...
## 0-IMP-1. ✅ HECHO (2026-08-17): EL IMPORT DE PESO ACEPTA EL EXPORT REGIONAL DE GARMIN

**Síntoma (owner):** subir el CSV de peso desde la web daba *"No se pudo reconocer el tipo de CSV"*. Bloqueaba toda la carga de peso/composición corporal (web y móvil comparten el parser del backend). Card Kan `24ytx6bqkrw4` (P0).

**Cómo falla la costura:** el web (`web/src/upload/importFile.ts`) prueba weight → steps → sleep contra `/metrics/import/<tipo>/parse`; un 400 significa "no es este tipo, probá el siguiente". Si los tres dan 400, muestra *"No se pudo reconocer el tipo de CSV"*. O sea: el mensaje del web **oculta** el error real del parser de peso (que sí es el correcto para el archivo).

**Causa raíz — dos formatos regionales, no uno.** El diagnóstico del ticket vio solo la mitad; la otra **apareció al mirar el `Weight.csv` real del owner** (ver [[testear-la-costura]]):
1. **Fecha `D MMM YYYY`** (`" 16 Aug 2026"`, día primero, sin coma) vs. el US `MMM D, YYYY` (`Aug 16, 2026`). `DATE_ROW` solo matcheaba el US → `curDate=null` → todo se saltea → `rows=0` → 400. **Fix [#237](https://github.com/thusspokedata/pulsia/pull/237)** (`fac`… squash): `DATE_ROW_MDY` + `DATE_ROW_DMY` + helper `matchDateRow` que devuelve el token de mes **sin resolver** (preserva el skip "Mes no reconocido").
2. **Hora 24h sin AM/PM** (`10:54`, `23:54`, `09:04`) vs. el US `10:54 AM`. `parse12hTime` (`backend/src/metrics/csvUtils.ts`) exigía `AM|PM` → aun con el fix de fecha, **cada medición se salteaba por "Hora inválida"** → `rows=0` → 400. **Fix [#238](https://github.com/thusspokedata/pulsia/pull/238)**: `parse12hTime` → **`parseClockTime`** (acepta 12h con AM/PM **y** 24h `0–23`); único call-site en `parseWeightCsv.ts`.

**Verificación:** e2e contra el `Weight.csv` real (**fuera del repo**, [[nunca-datos-reales-en-el-repo]]) → **8/8 filas, 0 skipped**, las 5 métricas por fila. TDD con fixtures **sintéticos** + verificación por mutación en ambos PRs. Ambos `@claude review` aprobados (0 bloqueantes; observaciones menores absorbidas: comentario del orden MDY/DMY). **Backend deployado** a la Pi (auto en push a `main`, health ✓; `parseClockTime` verificado con `grep` **dentro del contenedor**). **Sin OTA** (backend puro; el móvil ya postea el CSV igual).

**Fuera de alcance → tarjeta aparte `IMP-1b` (P2 · Quick wins):** el export **degenerado** `Weight(1).csv` (header + una sola medición **sin ninguna fila-fecha**) sigue dando error — decisión de producto pendiente del owner: *error claro* vs. *sin-fecha ⇒ hoy*. Ese fix no entra en IMP-1.

**Gotcha de sesión (no repetir):** ramifiqué con otra sesión trabajando WIP en el **mismo working tree** → ver [[git-snapshot-stale-concurrent-session]] (chequear `git status -sb` antes de `checkout -b`). Y **`rtk` reescribe/trunca `curl`** → para la REST API de Kan usar `rtk proxy curl` ([[kan-kanban-pi]]).

## 0-PROG-8. ✅ HECHO (2026-08-16): PROGRESO — DEPURAR EL SELECTOR DE MÉTRICAS + DECIMALES EN EL REGISTRO DIARIO

Sigue a **PROG-7** (formateo). Cuatro retoques al tab **Progreso** en un solo PR ([#235](https://github.com/thusspokedata/pulsia/pull/235), squash `0d2076a`), TDD → `@claude review` (aprobó, **0 bloqueantes**) → backend deployado (health ✓) + **OTA a `preview` runtime `11`**. Backlog en **Kan** ([[kan-kanban-pi]]), card `rk1aieqkor1p`. El único punto con **decisión de producto** lo resolvió el owner. Estado en [[prog-8-metric-selector-status]].

### (1) `sleep_quality` → bloque subjetivo "Cómo te sentís" (decisión del owner)
Es un **rating 1-5 manual** que el owner no usa (Garmin ya da el "Puntaje de sueño" /100). Pasa de `ACTIVITY_METRIC_TYPES` a `SUBJECTIVE_METRIC_TYPES` en `shared/schemas/metrics.ts`. Los **mapas keyed** (`METRIC_UNITS`/`LABELS`/`RANGES`) no cambian → **los datos viejos siguen validando** (sigue en `METRIC_TYPES`). Efecto secundario buscado: su input se mueve de "Actividad y recuperación" a "Cómo te sentís" en el Registro diario. El import de CSV de Garmin **nunca** produjo `sleep_quality` (test `parseSleepCsv` sigue verde).

### (2) Tendencia sin "Objetivo de pasos" (`steps_goal`)
`steps_goal` es un **target**, no una serie a seguir → fuera de los chips de Tendencia. Nuevo **`TREND_METRIC_TYPES`** = actuales sin `steps_goal`. Como "valor actual" sí se sigue mostrando.

### (3) Ocultar métricas SIN datos (Valores actuales Y Tendencia)
Helper **`metricsWithData(types, latest)`** deja solo las que tienen ≥1 dato (presentes en `latest`) → adiós tiles "—" y chips vacíos. **Guard** (`useEffect` sobre `latest`): si `selected` cae en una métrica oculta, reselecciona la primera de tendencia con datos (evita el chart vacío sin chip resaltado).

### (4) Decimales sin redondear en el Registro diario
`valuesForDay` precargaba los inputs con `String(last.value)` → el owner veía `Sueño (h): 10.7833333`. Ahora usa **`formatMetricValue`** (≤2 decimales). Es el sitio que **PROG-7 no cubrió**. Como es un input editable, el valor redondeado es el que se reguardaría (corrige la precisión almacenada, deseable).

### Verificación + review
TDD (test en rojo → verde) en los 6 archivos, **verificado en disco** (no solo el reporte del subagente): shared metrics + mobile `metricForm`/`progreso` + typecheck de shared/mobile/backend, todo en verde. El `@claude review` verificó los 4 puntos + el guard, confirmó que los datos viejos validan y que no hay call-sites huérfanos; solo **2 notas no bloqueantes** (el input relocalizado —buscado— y un comentario sobre por qué el `useEffect` omite deps). CodeRabbit solo dejó su boilerplate "review on request" (repo <10 stars).
