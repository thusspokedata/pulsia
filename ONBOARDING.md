...
> Documento de contexto para retomar el proyecto en una sesión nueva. Última actualización: **2026-07-17** (sesión: **DOMINIO 2 — NUTRICIÓN COMPLETO**: suplementos #3 (los 3 PRs), **pieza C** (dashboard MFP con 4 pestañas + torta + dona + referencias OMS + "alimentos con más X"), **evolución del nutriente en el tiempo**, y **alta de alimentos por texto** — escribir "almendra" en vez de fotografiarla — con trazabilidad etiqueta/estimado. Detalle en §0-HOY). Sesión previa 2026-07-14/15: el resto del dominio Nutrición (registro de comidas, balance energético, informes del agente) — §0-ANTES. Todo en `main`, backend deployado, **APK vc10** activado, todo lo nuevo por **OTA a vc10**. **Fingerprint vc10 = `784872cb…`** (ver [[ota-fingerprint-gotcha]]). **1 PR abierto** (#116, bump de dependencia de Dependabot, sin tocar). **Prod sano.**
...
## 0-HOY. ✅ HECHO esta sesión (2026-07-16/17): DOMINIO 2 — NUTRICIÓN COMPLETO
...
- **Pasarle mutación a los tests que YA existen.** De los 5 tests falsos que aparecieron hoy, **2 estaban en `main` desde hacía meses** y salieron de casualidad porque toqué esos archivos. Nadie sabe cuántos más hay. Ver la lección en §0-HOY. **Avance (2026-07-17, [#142](https://github.com/thusspokedata/pulsia/pull/142)):** auditado `shared/src/supplements/` — no eran tests falsos sino **4 huecos de cobertura** (lógica que ningún test ejercitaba: el invariante `SCAN_DAYS=14` = LCM de los períodos de frecuencia, la guarda de clave vacía en `overlap.ts`, `SupplementSchema.id` como UUID, y 9 de los 11 prefijos de `GENERIC_PREFIXES` (los otros 2, `vitamina` y `omega`, ya tenían test)). Cerrados con 12 tests, cada uno verificado por mutación. **Falta** pasarle mutación al resto del código.
