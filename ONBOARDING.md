...
> Documento de contexto para retomar el proyecto en una sesión nueva. Última actualización: **2026-07-13** (sesión: **fix del descifrado de ECG (`qpdf` #108, era el bug real, no el %PDF/timeout que decía el handoff) + peso del perfil single-source (#110) + registro diario prefill por día (#112) + REDISEÑO visual "clínico fresco" teal (#111/#113)**. Fingerprint vc9 **confirmado** = `410b46bf…`. Detalle en §0-HOY. Sesión previa 2026-07-12: **feature ECG (AliveCor KardiaMobile 6L) → APK vc9** — subir PDFs de ECG + interpretación con IA (Opus 4.8, no-diagnóstica, anclada a Kardia) + historial longitudinal + toggle en Configuración. Antes en la misma tanda: vc8 (campana en background) + `/download` con QR + fixes de sesión + nombres en español + datos de actividad/salud + peso single-source + sexo). Todo en `main`, backend deployado, **APK vc9 released + activado**. **Fingerprint vc9 = `410b46bf…`** (ver §0-hoy). **0 PRs abiertos, prod sano.**

## 0. Estado en una línea

**Pulsia está EN INTERNET, multi-usuario, con login.** Backend en **`https://pulsia.lahuelladelcaminante.de`** (VPS nginx → Wireguard → Pi:3011, HTTPS por certbot, rate-limit en `/auth/`). La app (Android, **APK vc9** con login + campana en background; todo lo nuevo llega por **OTA** a vc9) registra/resume/revisa entrenamientos, HR por banda BLE, resumen con mapa corporal + FC por ejercicio + curva de FC de la sesión, nombres de ejercicios en español + inglés, notas, sustitución, memoria del atleta, % cumplimiento, peso sugerido, entreno puntual expandido, genera programas async, un tab **"Progreso"** con seguimiento cuantitativo (composición corporal + presión arterial + actividad/recuperación: pasos/pisos/sueño/FC reposo + bienestar: estrés/ánimo/energía, con backfill por fecha) + tendencias + heatmap anual, y una sección **"ECG (KardiaMobile)"** (activable en Configuración): subir PDFs del ECG → **interpretación con IA (Opus 4.8, no-diagnóstica, anclada al veredicto de Kardia)** + historial longitudinal. **La IA observa al generar** todo esto (progreso, actividad/sueño como promedio + alertas, veredictos de ECG). El perfil tiene sexo (opcional) y el peso es single-source (canónico desde Progreso). Owner: `a.saleme@pm.me`. La familia baja el APK **vc9** desde **`pulsia.lahuelladelcaminante.de/download`** (QR) + se registra con el **`INVITE_CODE`**. Un merge a `main` **auto-deploya el backend a la Pi**.

## 0-HOY. ✅ HECHO esta sesión (2026-07-13): fix ECG (qpdf) + peso single-source + registro diario + REDISEÑO visual
...
Bumpear CUALQUIER dep del móvil (hasta un devDep) cambia el `runtimeVersion` (fingerprint) y **rompe el OTA hacia el APK instalado**. **ACTUALIZADO (vc9, CONFIRMADO 2026-07-13):** la versión activa es **vc9 = `410b46bfe3162b4562fc7c42737b455d824a885c`** (re-basado por las deps de ECG: `expo-document-picker` + `expo-file-system` + `expo-sharing`). **Verificar SIEMPRE** que `eas update` reporte runtime android **`410b46bf…`** (ya NO `88cc46dd` de vc8, ni `aeaa36d9` de vc7). El `88cc46dd` que se había anotado como fingerprint de vc9 era una predicción errada — el valor real (verificado por OTA que llegó al teléfono) es `410b46bf`. Los que sigan en vc7 no reciben OTAs de 88cc46dd hasta instalar vc8. `.github/dependabot.yml` ignora las deps del móvil (typescript, react, react-native-*, expo*, async-storage, babel major, jest major).
